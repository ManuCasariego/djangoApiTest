...
def beRoomersAccommodations(filters=None):
    if filters is None:
        filters = {}
    ...
    route = '../data/accommodations_ber_' + city.replace('-', '_') + '.json'
    ...
    for accommodation in accommodations:
        ...
        price = int((price.replace('€', '').replace('$', '').replace('£', '')).split('.')[0])
